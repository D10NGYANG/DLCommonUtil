...
fun getDistanceOn2Points(point1: Coordinate, point2: Coordinate, highPrecision: Boolean = false): Double {
    ...
    val b = toRadians(point1.lng) - toRadians(point2.lng)
    val sa2 = sin(a / 2)
    val sb2 = sin(b / 2)
...
/**
 * 将角度转换为弧度
 * @param degree [Double] 角度
 * @return [Double] 弧度
 */
fun toRadians(degree: Double) = degree * PI / 180

/**
 * 将弧度转换为角度
 * @param radian [Double] 弧度
 * @return [Double] 角度
 */
fun toDegrees(radian: Double) = radian * 180 / PI
...
fun getAngleOn2Points(point1: Coordinate, point2: Coordinate, highPrecision: Boolean = false): Double {
    if (highPrecision) {
        val bearing = getDistanceAndBearing(point1, point2).finalBearing
        return if (bearing < 0) bearing + 360 else if (bearing == 180.0) 0.0 else bearing
    }
...
fun getPointOn2Points(point1: Coordinate, point2: Coordinate, present: Float): Coordinate {
    ...
    if (present >= 1) return point2.copy()
...
fun getNearPartsOnLine(point: Coordinate, line: Array<Coordinate>, offset: Float = 10f): Array<Int> {
    if (line.size < 2) throw RuntimeException("轨迹至少需要两个点")
...
fun getProjectionPointOnLinePart(point: Coordinate, part: Array<Coordinate>): Coordinate {
    if (part.size != 2) throw RuntimeException("线段必须是两个点")
    val start = part[0]
    val end = part[1]
